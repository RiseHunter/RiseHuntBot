...
PV_CATEGORIES = {
    "pv_novice": {
        ...
        "label": "Новичок",
        "desc":  (
            "Активный образ жизни — зарядка, прогулки, иногда зал или бассейн.\n"
            "Стаж регулярных тренировок: *до 1 года* или занимаешься нерегулярно.\n"
            "Цель — поддерживать здоровье и форму."
        ),
        "url":   "https://tally.so/r/ССЫЛКА_НОВИЧОК",   # ← замените
        "range": (0, 100),     # диапазон баллов Tally
        "pv_min": 0.1,         # минимальный PV этой категории
        "pv_max": 4.5,         # максимальный PV этой категории
    },
    "pv_amateur": {
        "emoji": "💛",
        "label": "Любитель",
        "desc":  (
            "Тренируешься регулярно *2–4 раза в неделю, 1–4 года*.\n"
            "Знаешь технику базовых упражнений, следишь за питанием.\n"
            "Спорт — важная часть жизни, но не главная."
        ),
        "url":   "https://tally.so/r/ССЫЛКА_ЛЮБИТЕЛЬ",  # ← замените
        "range": (0, 100),
        "pv_min": 3.5,
        "pv_max": 6.5,
    },
    "pv_warrior": {
        ...
        "label": "Воин",
        "desc":  (
            "Полупрофессиональный уровень. Тренируешься *4–6 раз в неделю, 5+ лет*\n"
            "в силовом виде спорта с задействованием всех основных мышечных групп\n"
            "(пауэрлифтинг, кроссфит, борьба, тяжёлая атлетика, гимнастика).\n"
            "Есть спортивные разряды или участие в соревнованиях."
        ),
        "url":   "https://tally.so/r/ССЫЛКА_ВОИН",      # ← замените
        "range": (0, 100),
        "pv_min": 5.5,
        "pv_max": 8.5,
    },
    "pv_elite": {
        ...
        "label": "Элита",
        "desc":  (
            "Профессиональный спортсмен или бывший профессионал. *10+ лет* системных\n"
            "тренировок. Уровень МС/МСМК или эквивалент в своей дисциплине.\n"
            "Тренировочный процесс полностью структурирован под пиковую форму."
        ),
        "url":   "https://tally.so/r/ССЫЛКА_ЭЛИТА",     # ← замените
        "range": (0, 100),
        "pv_min": 7.5,
        "pv_max": 10.0,
    },
...
def pv_convert(raw: float, cat_key: str) -> float:
    """
    Переводит балл теста Tally (0–100) в шкалу PV бота
    с учётом диапазона категории.

    Новичок:     0–100 → 0.1–4.5
    Любитель:    0–100 → 3.5–6.5
    Воин:        0–100 → 5.5–8.5
    Элита:       0–100 → 7.5–10.0
    """
    cat   = PV_CATEGORIES[cat_key]
    lo    = cat["pv_min"]
    hi    = cat["pv_max"]
    score = lo + (raw / 100) * (hi - lo)
    return round(max(0.1, min(10.0, score)), 1)


def pv_label(score: float) -> str:
    if score >= 9.5: return "⚫ Экстремальная витальность (Воин/Элита)"
    if score >= 8.5: return "🔴 Очень высокая"
    if score >= 7.0: return "🟠 Высокая"
    if score >= 5.0: return "💛 Средняя"
    if score >= 3.0: return "💚 Низкая"
    return "⬇️ Критическая витальность"
...
def callback_handler(call):
    ...
    try:
        ...
        # ── Registration finish ───────────────────────────────────────────────
            ...
            m = InlineKeyboardMarkup(row_width=1)
            # PV — отдельная кнопка (у неё свой обработчик с категориями)
            m.add(InlineKeyboardButton(
                "💪 PV — Физическая витальность",
                callback_data="test_PV"
            ))
            # Остальные тесты из TESTS_CONFIG
            ...
            old = u["PV"]
            m = InlineKeyboardMarkup(row_width=1)
            for cat_key, cat in PV_CATEGORIES.items():
                m.add(InlineKeyboardButton(
                    f"{cat['emoji']} {cat['label']}",
                    callback_data=f"pv_cat_{cat_key}"
                ))
            m.add(InlineKeyboardButton("🔙 Назад", callback_data="tests_menu"))
            edit(
                f"💪 *PV — Физическая витальность*\n\n"
                f"Текущий уровень: `{old:.1f}/10` {bar(old)}\n\n"
                "Выбери свою категорию подготовки:",
                m,
            )

        elif data.startswith("pv_cat_"):
            cat_key = data[7:]
            if cat_key not in PV_CATEGORIES:
                bot.answer_callback_query(call.id, "Неизвестная категория")
                return
            cat = PV_CATEGORIES[cat_key]
            user_states[user_id] = {"type": "pv_input", "cat_key": cat_key}
            m = InlineKeyboardMarkup(row_width=1)
            m.add(
                InlineKeyboardButton(f"🌐 Пройти тест — {cat['label']}", url=cat["url"]),
                InlineKeyboardButton("🔙 Выбрать категорию", callback_data="test_PV"),
            )
            edit(
                f"{cat['emoji']} *{cat['label']}*\n\n"
                f"{cat['desc']}\n\n"
                "━━━━━━━━━━━━━━━━━━━━\n"
                "Пройди тест по ссылке выше.\n"
                "Tally покажет *балл от 0 до 100*.\n\n"
                "✏️ Вернись и введи этот балл:",
                m,
            )
...
def handle_text(message):
    ...
    # ── Edit name ─────────────────────────────────────────────────────────────
        ...
        try:
            ...
            bot.reply_to(message, "❌ Введи число от *0 до 100*.",
                         parse_mode="Markdown", reply_markup=kb_back_main())
            return
        if not (0 <= raw <= 100):
            bot.reply_to(message, "❌ Балл должен быть от *0 до 100*.",
                         parse_mode="Markdown", reply_markup=kb_back_main())
            return
        u = get_user(user_id)
        old_val = u["PV"]
        new_val = pv_convert(raw, cat_key)
        update_direction(user_id, "PV", new_val)
        change = "📈" if new_val >= old_val else "📉"
        bot.reply_to(
            message,
            f"✅ *PV обновлено!*\n\n"
            f"💪 {pv_label(new_val)}\n"
            f"📋 Категория: {cat['emoji']} {cat['label']}\n\n"
        ...
        del user_states[user_id]
        check_and_level_up(message.chat.id, user_id, "PV", new_val)
